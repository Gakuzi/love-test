...
function ensureReadmeSheet_() {
  ...
  sh.clear();
  const notes = [
    ['Инструкция по редактированию и расчётам'],
    ['СТРУКТУРА ЛИСТОВ:'],
    ['Вопросы: id, block, blockIndex(0..3), text, hint, options(JSON массив {value,label})'],
    ['Рекомендации: blockIndex(0..3), zone(success|warning|danger), title, text'],
    ['План: blockIndex(0..3), timeframe(период), title, description (для плана действий)'],
    ['Формулы: param, value, comment — для документирования логики порогов (используется как справочник)'],
    ['Результаты/Результаты по вопросам: заполняются приложением автоматически'],
    ['События: дата, UUID, ref, event, payload(JSON) — лог взаимодействий пользователя'],
    [''],
    ['API ВЕБ-ПРИЛОЖЕНИЯ:'],
    ['GET ?action=config — возвращает { ok, config:{questions}, recommendations, plan }'],
    ['GET ?action=init&token=SHARED_TOKEN — создаёт листы и заполняет их дефолтными данными'],
    ['POST {action:"analyze", answers:{index:value}} — возвращает { ok, analysis:{blockResults, overall, priorityBlock} }'],
    ['POST без action — пишет строки в Результаты/Результаты по вопросам и События при наличии поля event'],
    [''],
    ['ГДЕ СЧИТАЮТСЯ РЕЗУЛЬТАТЫ:'],
    ['— По умолчанию расчёт выполняется на сервере (action=analyze) и дублируется локально в клиенте на JS как подстраховка.'],
    ['— Зоны блоков: sum пяти вопросов → success (>=11), warning (>=6), иначе danger.'],
    ['— Общее состояние: среднее по блокам <8 → разрушительные, <11 → шаткие, иначе зрелые.'],
    [''],
    ['КАК РЕДАКТИРОВАТЬ:'],
    ['— Добавьте/измените вопросы на листе Вопросы (колонки строго по заголовкам).'],
    ['— Добавьте рекомендации для каждой зоны на листе Рекомендации.'],
    ['— Добавьте шаги плана по приоритетным блокам на листе План.'],
    ['— Нажмите init (GET?action=init&token=...) или параметр seed=1 в URL клиента для первичного создания листов.']
  ];
